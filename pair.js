...
const SUDO_FILE_PATH = path.join(__dirname, 'sudo.json');

const loadSudoNumbers = () => {
    try {
        if (fs.existsSync(SUDO_FILE_PATH)) {
            const data = fs.readFileSync(SUDO_FILE_PATH, 'utf-8');
            SUDO_NUMBERS_LIST = JSON.parse(data);
            console.log(`👤 Loaded ${SUDO_NUMBERS_LIST.length} Sudo numbers.`);
        } else {
            // Créer le fichier s'il n'existe pas
            fs.writeFileSync(SUDO_FILE_PATH, '[]');
            SUDO_NUMBERS_LIST = [];
            console.log("Created empty sudo.json.");
        }
    } catch (e) {
...
// Exécuter le chargement des Sudous au démarrage
loadSudoNumbers();
// -------------------------------------------------------------
...
> 𝑵𝑶𝑿 𝑴𝑰𝑵𝑰 𝑩𝑶𝑻`;
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                // --- VÉRIFICATION SUDO AVEC LA LISTE CHARGÉE ---
                // Le Sudo est soit dans la liste des numéros, soit il est l'Owner
